                ...
                if (len(rows)<=0) :
                    ...
                    if sys_args['logdb_mode'] == '1':           # mdb
                        ...
                        if sys_args['logdb_mode'] == '2':       # postgreSQL
                            _sql = "INSERT INTO sde.sde.flowctrl (objectid,zipfilename,priority,progress,status,imagebits,refertime)"
                            _sql += " VALUES( (select COALESCE(MAX(objectid), 0)+1 from sde.sde.flowctrl)"
                        ...
                        # 否則即是 ms sql server
                            ...
                            _sql = "INSERT INTO sde.sde.flowctrl (objectid,zipfilename,priority,progress,status,imagebits,refertime)"
                            _sql += " VALUES( (select isnull(MAX(objectid), 0)+1 from sde.sde.flowctrl)"
                            _sql += ",'"+zip_name+"',"+sys_args['defaultPriority']
                            _sql += ",'0:待轉','0:未處理',16,'"+theTimeS+"')"
                            sr.execute(_sql)
                    if (sr.rowcount <= 0) :
...
def FlowCtrl_StatusMsg(zip_name, status, add_msg) :
    ...
    if add_msg != '' :
        ...
        sr.execute(_sql)
        rows = sr.fetchall()
        msg = rows[0][0] + ';' + add_msg.replace("'"," ").replace('"',' ')
