...
#////////////////////////////////////////////////////
# 將 image 新增到 gdb/raster dataset
...
def getMetaDataPath( sour_imagePath ) :
    ...
    if nowImage_args['rasterType_name'] in ['GeoEye-1'] :
        ...
        #因 add raster 圖名自動給 metadata檔名，此不可改，故能用 metadata 檔名
        ...
        with lock_dir.acquire(timeout=30):
            for zip_name in arr_zip:
                ...
                if (len(rows)<=0) :
                    ...
                    # 若是 progress 99: 已轉過則自動重轉
                    if rows[0][1].find('99:') != -1 :
                        rows[0][1] = '0:待轉'
                        rows[0][2] = '0:未處理'
                        _sql = "update FlowCtrl set Progress='"+rows[0][1]+"',Status='"+rows[0][2]+"'"
                        _sql += " where ZipFileName='" + zip_name + "'"
                        sr.execute(_sql)
                         
                    # 處理 fgdb 同星種不可同時轉
                    if sys_args['is_sdeDB'] != 'Y' and sys_args['is_MultiRun'] == 'Y' :
                        # progress 非 0 且 status 非 3 的星種保留，表示此星種正轉換中，不可轉
...
# (ps: 此將來可能圖名也要換)
def FlowCtrl_Step_5_MosaicDataset(zip_name,mosaic_dataset,image_name,raster_id) :
...
def FlowCtrl_Step_9(zip_name) :
    ...
    cn.close()
    return True
# 此與 16bit mosaic dataset 相同，要加更新 raster_id 欄
# (ps: 之後同樣可能要加更新圖名)
def FlowCtrl_Step_9_MosaicDataset(zip_name,mosaic_dataset,image_name,raster_id) :
    cn = pyodbc.connect(sys_args['flowConnectStr'],autocommit=True)
    sr = cn.cursor()
...
# 將 zip 檔移動到 zip 保留區
def move_zip_to_stub(sour_zip,zip_name) :
    ...
    # zip已存在先刪除
    if os.path.exists(move_path+'/'+zip_name):
        os.remove(move_path+'/'+zip_name)

    fprint( '轉完，將 zip 檔移動到:'+move_path+'/'+zip_name+'\n' )
    shutil.move( sour_zip, move_path+'/'+zip_name )
...
def main():
    ...
    while process_zip_count < sys_args['limit_doFiles'] :
       ...
       try:
           ...
           step_errmsg = '解壓縮錯誤'
           # 解壓縮此zip到 Temp 路徑
           tempzip_dir_root = tempZip_path + os.path.splitext(zip_name)[0]
           ...
           if zip_name.endswith(".zip"):      # .zip 用標準方式
               ...
               #改用patool
               patoolib.extract_archive(sys_args['zips_path'] + zip_name, outdir=tempzip_dir_root)

           # 去除多餘路徑取得正確 tempzip_dir
           tempzip_dir = checkZipRoot(tempzip_dir_root,os.path.splitext(zip_name)[0])
           ...
           if zip_ImageName == '' :
               ...
               FlowCtrl_StatusMsg(zip_name, '3:失敗', '找不到 metadata')
               continue
           ...
           if nowImage_args['bits'] == '16' :
               ...
               # mosaic dataset 要加寫屬性(目前僅多一欄RasterID，之後要 parser metadata 檔各屬性放入)
               ...
               # 完成後將 zip 檔移動到zip保留區
               move_zip_to_stub(sys_args['zips_path']+zip_name,zip_name)
