...
# 回復狀態 0:待辦
def FlowCtrl_Step_0(zip_name) :
    cn = pyodbc.connect(sys_args['flowConnectStr'])
    sr = cn.cursor()
    _sql = "UPDATE FlowCtrl SET Progress='0:待轉',Status='0:未處理',RasterTypeId='',RasterType='',StartTime='',EndTime='',ErrMsg=''"
    ...
    while process_zip_count < sys_args['limit_doFiles'] :
       ...
       try:
           ...
           if nowImage_args['bits'] == '16' :
               ...
               tempRaster_16 = sys_gdb+'/R'+datetime.now().strftime("%y%m%d_%H%M%S")
               ...
               # 轉16bit到8bit
               tempRaster_8 = sys_gdb+'/R'+datetime.now().strftime("%y%m%d_%H%M%S")
