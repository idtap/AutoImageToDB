...
##############################################################################################################
# 2022/08/04 備考
#   1.此版本測試當新圖匯入成功後進行 extent 查詢訂單，無則查詢目標發 e-mail 及增加一筆編報任務
#   2.預計先跑一個完整 zip 匯入，不設為成功失敗不刪前端，之後改成此檔一直重複最後一步來測試
...
import requests
word_args = {
    'connectStr'    : 'DSN=PostgreSQL35W_83',
    'acq_time_list' : ["acq_time","earliestacqtime","acquisitiondate","time_range","acquired"],
    'token_url'     : 'https://demo.igis.com.tw/portal/sharing/rest/generateToken',
    'token_user'    : 'saga',
    'token_pass'    : 'shryu7041',
    'token_refer'   : 'https://demo.igis.com.tw/',
    'order_url'     : 'https://demo.igis.com.tw/server/rest/services/SampleWorldCities/MapServer/2/query',
    'target_url'    : 'https://demo.igis.com.tw/server/rest/services/SampleWorldCities/MapServer/0/query'
}
...
# 初始化 arcpy env
#這個會變動作不循序
#print('processing:'+str(arcpy.env.parallelProcessingFactor))
#arcpy.env.parallelProcessingFactor = "100%"
#這個確是不產生 ovr 檔，但對顯示會不對
#print('pyramid:'+str(arcpy.env.pyramid))
...
def checkOther( check_path ):
    ...
    # 開啟 json 找關鍵字(此處可暫不 json parser，當一般 txt find 即可)
    pos = findFileStrPos( meta_file_name, 'img_id')   # 此檔中需有此才是

    # pos>-1 則找到，找到後依 id 由 config 讀取相關資料
    bo = False
    if pos>-1 :
        bo = getConfigById('Other01')            # 回傳星種 id
...
def checkImageType(image_root_path):
    ...
    # 20211017 
    if not bo : bo = checkOther( image_root_path )              # 找其他 8 bit
...
def getMetaDataName( sour_imagePath ) :
    ...
    # 20211017
    # Other
    if nowImage_args['rasterType_name'] in ['Other'] :
        # 先找 MUL 主圖檔
        bo = False
        for root, dirs, files in walk(sour_imagePath):
            for f in files:
...
# 20211017
# paraser Other 屬性
def parserOtherXML( xml_file_name ) :
    ...
    # 讀取 img_id
    image_MetaData['img_id'] = getXmlTagValue(root,'./img_id')
    print( 'img_id:'+image_MetaData['img_id'] )
    image_MetaData['acq_time'] = getXmlTagValue(root,'./acq_time').replace('T',' ')
    print( 'acq_time:'+image_MetaData['acq_time'] )
    image_MetaData['sun_elev'] = getXmlTagValue(root,'./sun_elev')
    print( 'sun_elev:'+image_MetaData['sun_elev'] )
    image_MetaData['sun_azu'] = getXmlTagValue(root,'./sun_azu')
    print( 'sun_azu:'+image_MetaData['sun_azu'] )
    image_MetaData['cloud_rate'] = getXmlTagValue(root,'./cloud_rate')
    print( 'cloud_rate:'+image_MetaData['cloud_rate'] )
    image_MetaData['band'] = getXmlTagValue(root,'./band')
    print( 'band:'+image_MetaData['band'] )
    image_MetaData['cen_x'] = getXmlTagValue(root,'./cen_x')
    print( 'cen_x:'+image_MetaData['cen_x'] )
    image_MetaData['cen_y'] = getXmlTagValue(root,'./cen_y')
    print( 'cen_y:'+image_MetaData['cen_y'] )
    image_MetaData['ul_x'] = getXmlTagValue(root,'./ul_x')
    print( 'ul_x:'+image_MetaData['ul_x'] )
    image_MetaData['ul_y'] = getXmlTagValue(root,'./ul_y')
    print( 'ul_y:'+image_MetaData['ul_y'] )
    image_MetaData['ur_x'] = getXmlTagValue(root,'./ur_x')
    print( 'ur_x:'+image_MetaData['ur_x'] )
    image_MetaData['ur_y'] = getXmlTagValue(root,'./ur_y')
    print( 'ur_y:'+image_MetaData['ur_y'] )
    image_MetaData['ll_x'] = getXmlTagValue(root,'./ll_x')
    print( 'll_x:'+image_MetaData['ll_x'] )
    image_MetaData['ll_y'] = getXmlTagValue(root,'./ll_y')
    print( 'll_y:'+image_MetaData['ll_y'] )
    image_MetaData['lr_x'] = getXmlTagValue(root,'./lr_x')
    print( 'lr_x:'+image_MetaData['lr_x'] )
    image_MetaData['lr_y'] = getXmlTagValue(root,'./lr_y')
    print( 'lr_y:'+image_MetaData['lr_y'] )
    image_MetaData['sat_type'] = getXmlTagValue(root,'./sat_type')
    print( 'sat_type:'+image_MetaData['sat_type'] )
    image_MetaData['catalog_id'] = getXmlTagValue(root,'./catalog_id')
    print( 'catalog_id:'+image_MetaData['catalog_id'] )
    image_MetaData['receive_time'] = getXmlTagValue(root,'./receive_time').replace('T',' ')
    print( 'receive_time:'+image_MetaData['receive_time'] )
        ...
        for root, dirs, files in walk(check_path):
            for f in files:
                ...
                if f.endswith('_metadata.xml'):                 
                    xml_name = fullpath
        if os.path.exists(xml_name) :
            parserOtherXML(xml_name)
        else:
            step_errmsg = 'xxx_metadata.xml 檔不存在，無法萃取 MetaData'
            return False
...
def getNextZip() :
    ...
    return "Tripoli_View-Ready_8_Band_Bundle_30cm"
    ...
    try:
        with lock_dir.acquire(timeout=30):
            for zip_name in arr_zip:
                ...
                # 20211027 判斷此 zip 是否正寫入中，是則
                idx = arr_zip.index(zip_name)
                full_zip_name = arr_zip_f[idx]
                if is_use(sys_args['zips_path']+full_zip_name) :
                    continue
...
def FlowCtrl_Step_5_MosaicDataset(zip_name,mosaic_dataset,image_name,raster_id,image_path) :
    ...
    for row in rows:
        ...
        if nowImage_args['rasterType_name'] in ['WorldView-1','WorldView-2','WorldView-3','WorldView-4','GeoEye-1'] and nowImage_args['rasterType_id'] != 'oldGE01':
            ...
            # 20211027 日期欄固定放 acq_time
            acq_time = image_MetaData['acq_time']
            dot_pos = acq_time.find('.')
            if dot_pos != -1 :
                acq_time = acq_time[:dot_pos]
            acq_time = datetime.strptime(acq_time, '%Y-%m-%d %H:%M:%S')
            row.setValue("acquisitiondate",     acq_time.strftime('%Y/%m/%d %H:%M:%S')      )
...
def FlowCtrl_Step_9_MosaicDataset(zip_name,mosaic_dataset,image_name,raster_id,image_path) :
    ...
    for row in rows:
        ...
        # Other
        if nowImage_args['rasterType_name'] in ['Other'] :
            row.setValue("img_id",              is_null(image_MetaData['img_id'],'')     )
            row.setValue("acq_time",            is_null(image_MetaData['acq_time'],'')   )
            row.setValue("sun_elev",            is_null(image_MetaData['sun_elev'],'0')   )
            row.setValue("sun_azu",             is_null(image_MetaData['sun_azu'],'0')    )
            row.setValue("cloud_rate",          is_null(image_MetaData['cloud_rate'],'0') )
            row.setValue("band",                is_null(image_MetaData['band'],'4')      )
            row.setValue("cen_x",               is_null(image_MetaData['cen_x'],'0')      )
            row.setValue("cen_y",               is_null(image_MetaData['cen_y'],'0')      )
            row.setValue("ul_x",                is_null(image_MetaData['ul_x'],'0')       )
            row.setValue("ul_y",                is_null(image_MetaData['ul_y'],'0')       )
            ...
            row.setValue("lr_x",                is_null(image_MetaData['lr_x'],'0')       )
            row.setValue("lr_y",                is_null(image_MetaData['lr_y'],'0')       )
            row.setValue("sat_type",            is_null(image_MetaData['sat_type'],'')   )
            row.setValue("catalog_id",          is_null(image_MetaData['catalog_id'],'') )
            row.setValue("receive_time",        is_null(image_MetaData['receive_time'],'')   )
...
# 20211002 rar 7z 解壓合併
def rar7z_merge(zip_file_name, tempzip_dir_root):
    # 先依 now_runtime 解壓至此
    temp_path = tempZip_path + sys_args['now_runtime']
    if os.path.isdir(temp_path):
        shutil.rmtree(temp_path)
    os.mkdir(temp_path)
    # 解壓至此
    patoolib.extract_archive(sys_args['zips_path'] + zip_file_name, outdir=temp_path)

    # 將檔案移到 tempzip_dir_root
    for root, dirs, files in walk(temp_path):
        for f in files:
            fullpath = join(root, f)
            shutil.copy(fullpath, tempzip_dir_root)
...
def clean_TEMP() :
    ...
    arr_sub = os.listdir(tempZip_path+'.')            # 取到 list 中
    for f_sub in arr_sub:
         # 取下此路徑
         dir_name = tempZip_path+f_sub
         if is_empty_dir(dir_name):
             try: 
                 shutil.rmtree(dir_name)
             except OSError as e: 
                 print('can not remove:'+dir_name)

    return True

#////////////////////////////////////////////////////////////////////////////////////////////
# 2022/08/04 增加額外後續處理編報系統介接

# 主程序
def wordflow_etc(image_name, mosaic_dataset, title):
    # 找出此 image extent 範圍，準備後續動作
    where_clause = "Name like '%"+image_name+"%'"
    extent_rings = ""
    with arcpy.da.SearchCursor(mosaic_dataset, ['SHAPE@'], where_clause) as cursor:
        for row in cursor:
            shape = row[0]
            d = json.loads(shape.JSON)
            #print("取得 ring ->\n{0}".format( '{"rings":'+str(d['rings'])+'}' ))
            extent_rings = '{"rings":'+str(d['rings'])+',"spatialReference":{"latestWkid":"4326","wkid":"4326"}}'
            break       # 只取一筆

    # 用此 ring query 訂單服務及目標服務(訂單找不到時)，找到後發 e-mail 給相關人
    print('接著以 extent 搜尋訂單')
    ...
    # 取得 token
    token_url = word_args['token_url']
    token_payload = {
        'username'   : word_args['token_user'],
        'password'   : word_args['token_pass'],
        'referer'    : word_args['token_refer'],
        'expiration' : 60,
        'f'          : 'JSON'
    }
    ...
    if token_response.status_code==200:
        ...
        if not ('error' in token_json) and ('token' in token_json):
            ...
            #print('取得 token:'+token_for_URL)

            # 以 polygon ring 查詢城市以此模擬安康以 extent 查訂單
            Query_URL = word_args['order_url']
            Query_payload = {
                'f'              : 'JSON',
                'token'          : token_for_URL,
                'where'          : '1=1',
                #'geometry'       : extent_rings,
                # 暫用寫死的 extent rings 代替
            ...
            if Query_response.status_code==200:
                ...
                if ('features' in query_json) and len(query_json['features'])>0 :
                    find_order = True
                    print( '找到訂單，對此訂單發 e-mail:'+str(query_json['features'][0]['attributes']) )
                    # 
                else:
                    print( '此範圍查無訂單' )
            else:
            ...
            #if not find_order:
            Query_URL = word_args['target_url']
            Query_payload = {
                'f'              : 'JSON',
                'token'          : token_for_URL,
                'where'          : '1=1',
            ...
            if Query_response.status_code==200:
                query_json = json.loads(Query_response.text)
                if ('features' in query_json) and len(query_json['features'])>0 :
                    print( '找到目標各筆，對此目標發e-mail:'+str(query_json['features']) )
                else:
                    print( '此範圍查不到任何目標點位' )
            else:
    ...
    # 有找到訂單時要自動新增一筆任務
    # 準備存放到 mission 任務 table 字串
    features_str = '{"features":[{"attributes":{' 

    # 取下此 MD 各欄(僅所需欄位型態)
    field_names = [
        f.name for f in arcpy.ListFields(mosaic_dataset)
        if f.name!="path" and (f.type=="Date" or f.type=="OID" or f.type=="String" or f.type=="Double" or f.type=="Integer" or f.type=="SmallInteger")
    ]

    # 搜尋 MD 此筆後組合各欄及值
    with arcpy.da.SearchCursor(mosaic_dataset, field_names, where_clause) as cursor:
        for row in cursor:
            # 各欄欄名及資料放入
            i = 0
            acq_time_ok = False
            for field in field_names:
                if i>0:
                    features_str += ','
                if field=="objectid":
                    features_str += '"'+field+'":'+str(row[i])
                elif (field in word_args['acq_time_list']) and not acq_time_ok:
                    acq_time_ok = True
                    acq_time = str(row[i])
                    dot_pos = acq_time.find('.')
                    if dot_pos != -1 :
                        acq_time = acq_time[:dot_pos]
                    acq_time = datetime.strptime(acq_time, '%Y-%m-%d %H:%M:%S')
                    features_str += '"acq_time":"'+str(int(acq_time.timestamp()))+'"'
                else:
                    features_str += '"'+field+'":"'+str(row[i])+'"'
                i = i + 1
            break          # 同樣只一筆
    # features 加一欄 geometry 放入上面的 ring
    features_str += '},"geometry":' + extent_rings + '}]}'
    #print(features_str)

    print('新增一筆任務到編報系統')
    # 取得的 rings 寫一筆到編報任務表格
    cn = pyodbc.connect(word_args['connectStr'],autocommit=True)
    sr = cn.cursor()
    _sql = "SELECT * FROM func_ins_mission('"+title+"',"+order_type+",'"+features_str+"')"
    sr.execute(_sql)
    sr.close()
    cn.close()
...
#           else:                              # 其他用 patool
#               #改用patool
#               # 20211002 改解壓及合併到 zips_path zip_file_name 路徑
#               rar7z_merge(zip_file_name, tempzip_dir_root)
#
           # 去除多餘路徑取得正確 tempzip_dir
...
#           FlowCtrl_Step_2(zip_name)
           ...
           if checkImageType(tempzip_dir):
              ...
              # rasterid 不判定直接寫
              nowImage_args['raster_id'] = '20220805WV030001'
...
#           FlowCtrl_StatusMsg(zip_name, '2:成功', '')
           ...
           # 依是否自行 pansharpen 處理(處理 BlackSky 等非制式星種)
...
#                  shutil.rmtree(file_dir)
               ...
               # 20211017 改 nas to nas, 此處看是否 t50 舊圖，是才可 move
               #nowImage_args['t50yearmonth'] = '202109'                          # 寫死測試
               if nowImage_args['t50yearmonth'] != '' :
                   t50_path = 'D:\\安康\\AutoImageToDB\\sourImage_T50\\'   # 亦暫寫死，應從系統參數
#                   shutil.move(t50_path+nowImage_args['t50yearmonth']+'/'+zip_name, nowImage_args['fileStore_16'])
                   # 如果是 self sharpen 還要多複製 \\AutoPan 下
#                   if nowImage_args['self_pansharpen'] == 'Y' and nowImage_args['pansharpen'] == 'Y':
#                       shutil.copytree(tempzip_dir + '\\AutoPan',file_dir+'\\AutoPan')
#               else:
...
#               move_zip_to_stub(sys_args['zips_path']+zip_file_name,zip_file_name)
        ...
        #
           ...
           # 2022/08/04 增加額外後續處理編報系統介接
           wordflow_etc(zip_ImageName, nowImage_args['gdbName'] + '\\' + nowImage_args['datasetName_16'], nowImage_args['raster_id'])
...
def addRunTimes() :
    ...
    try:
        with lock_times.acquire(timeout=30):
            ...
            sys_args['now_runtime'] = str(run_times)
