...
# 執行計次檔
run_times_file = sys_path + 'itas_runtimes.txt'

# 系統參數檔
syspara_path = sys_path + 'itas_sysparam_postgre.csv'
sys_args = {
    'logdb_mode'     : '1',                         # 監控LOG資料庫種類(1/mdb,2/postgre,3/SQL)
    'is_MultiRun'    : 'Y',                         # 多工執行
    'is_sdeDB'       : 'Y',                         # 使用 sde 資料庫
    'limit_doFiles'  : 1,                           # 每次執行處理幾個檔(配合每5分鐘執行一次，以控制同時執行數)
    'zips_path'      : sys_path + 'itas_input_zips/',    # zip 檔存放路徑
                                                    # 流程控制資料 connect string( MS-Access .mdb)
    'flowConnectStr' : r'DRIVER={Microsoft Access Driver (*.mdb, *.accdb)};DBQ=.\flowControl.mdb',
    'config_path'    : sys_path + 'itas_config_postgre.csv',     # 星種參數檔
    'defaultPriority': '9999',                          # 預設優先序
    'zips_stub'      : sys_path + 'itas_zips_stub/',         # zip 轉後留存路徑
    'sys_store'      : sys_path + 'connect_postgres.sde',       # 系統暫存用
...
# 系統暫存用路徑
tempZip_path     = sys_path + 'itas_tempZip/'
...
def fprint(w_str) :
    ...
    with open(sys_path+'itas_LOG.txt', 'a',encoding='UTF-8') as f:
...
#//////////////////////////////////////////////////////////////////////////////////////////////////////////
# 以下為各星種判斷 
# ps: 康華程式直接由 zip 名判斷後如 getConfigById('WV01')
def checkImageType(zip_name):
    raster_type = zip_name.split('_')[0]
    if raster_type == 'WV01':
        return getConfigById('WV01')            # 載入及回傳星種

    elif raster_type == 'PHR01':        # pleiades 不確定此名，暫定
        return getConfigById('PHR01')         

    elif raster_type == 'BS01':        # BlackSky 不確定此名，暫定
        return getConfigById('BS01')         

    elif raster_type == 'SKY01':       # SkySat 不確定此名，暫定
        return getConfigById('SKY01')         
    ...
    if nowImage_args['rasterType_name'] in ['WorldView-1'] :
        ...
        for root, dirs, files in walk(sour_imagePath):
            for f in files:
                ...
                if f.endswith( '.IMD' ):
                    imd_name = fullpath
                    if os.path.exists(imd_name) :
                        nowImage_args['pathMUL'] = imd_name
                        nowImage_args['pathPAN'] = imd_name
                        bo = True
                    break
            if bo :
                break
...
# 由itas_FlowCtrl管控表及路徑資料決定下一個待轉檔案，然後轉換
def getNextZip() :
    ...
    # 開啟 itas_FlowCtrl，逐筆查 itas_FlowCtrl 無此筆則新增
    ...
    lock_dir = FileLock("itas_check_dir.txt.lock")
    try:
        with lock_dir.acquire(timeout=30):
            for zip_name in arr_zip:
                ...
                _sql = "select ZipFileName,Progress,Status,RasterTypeID from itas_FlowCtrl"
                ...
                if (len(rows)<=0) :
                    ...
                    if sys_args['logdb_mode'] == '2':       # postgreSQL
                        _sql = "INSERT INTO itas_flowctrl (objectid,zipfilename,priority,progress,status,imagebits,refertime)"
                        ...
                        sr.execute(_sql)
                    if (sr.rowcount <= 0) :
                        fprint('無法新增紀錄到 itas_FlowCtrl，請檢查:'+zip_name)
                    sr.commit()
                else:    
                    ...
                    if rows[0][1].find('99:') != -1 or rows[0][2].find('3:') != -1 :
                        rows[0][1] = '0:待轉'
                        rows[0][2] = '0:未處理'
                        _sql = "update itas_FlowCtrl set Progress='"+rows[0][1]+"',Status='"+rows[0][2]+"',ErrMsg=''"
                        _sql += " where ZipFileName='" + zip_name + "'"
                        sr.execute(_sql)
            # 解鎖
            lock_dir.release()
    ...
    # 依優先序取得 itas_FlowCtrl 0:待轉 資料後取首筆優先轉
    _sql = "select ZipFileName,RasterTypeID from itas_FlowCtrl"
...
def FlowCtrl_Step_0(zip_name, rasterTypeID, rasterTypeName) :
    cn = pyodbc.connect(sys_args['flowConnectStr'],autocommit=True)
    sr = cn.cursor()
    _sql = "UPDATE itas_FlowCtrl SET Progress='0:待轉',Status='0:未處理',StartTime='',EndTime='',ErrMsg=''"
...
def FlowCtrl_Step_1(zip_name, start_time) :
    ...
    cn = pyodbc.connect(sys_args['flowConnectStr'],autocommit=True)
    sr = cn.cursor()
    _sql = "UPDATE itas_FlowCtrl SET Progress='1:獲優先開始轉',Status='1:處理中',StartTime='"+start_time+"'"
    _sql += " WHERE ZipFileName='"+zip_name+"'"
    sr.execute(_sql)
    sr.commit() 
    sr.close()
...
def FlowCtrl_Step_2(zip_name) :
    cn = pyodbc.connect(sys_args['flowConnectStr'],autocommit=True)
    sr = cn.cursor()
    _sql = "UPDATE itas_FlowCtrl SET Progress='2:星種判讀',Status='1:處理中'"
    _sql += " WHERE ZipFileName='"+zip_name+"'"
    sr.execute(_sql)
    sr.commit() 
    sr.close()
...
def FlowCtrl_Step_2_RasterType(zip_name, raster_type_id, raster_type, bits) :
    ...
    #年月日改由 zip_name 第2欄前8碼取
    time_s = zip_name.split('_')[1][:8]

    id_head = time_s+raster_type_id
    _sql = "select RasterID from itas_FlowCtrl"
    ...
    # 更新 flowctrl
    _sql = "UPDATE itas_FlowCtrl SET RasterTypeId='"+raster_type_id+"',RasterType='"+raster_type+"',ImageBits="+bits
...
def FlowCtrl_StatusMsg(zip_name, status, add_msg) :
    ...
    if add_msg != '' :
        _sql = "select ErrMsg from itas_FlowCtrl"
        _sql += " where ZipFileName='" + zip_name + "'"
        sr.execute(_sql)
        rows = sr.fetchall()
        if len(rows)>0 :
            ...
            #else:
            #    msg += add_msg
            _sql = "UPDATE itas_FlowCtrl SET Status='"+status+"',ErrMsg='"+msg+"'"
            _sql += " WHERE ZipFileName='"+zip_name+"'"
            sr.execute(_sql)
    else:
        _sql = "UPDATE itas_FlowCtrl SET Status='"+status+"'"
        _sql += " WHERE ZipFileName='"+zip_name+"'"
        sr.execute(_sql)
...
def FlowCtrl_Step_3(zip_name) :
    cn = pyodbc.connect(sys_args['flowConnectStr'],autocommit=True)
    sr = cn.cursor()
    _sql = "UPDATE itas_FlowCtrl SET Progress='3:屬性萃取',Status='1:處理中'"
    _sql += " WHERE ZipFileName='"+zip_name+"'"
    sr.execute(_sql)
    sr.commit() 
    sr.close()
...
def FlowCtrl_Step_3_ImageName(zip_name,image_name) :
    cn = pyodbc.connect(sys_args['flowConnectStr'],autocommit=True)
    sr = cn.cursor()
    _sql = "UPDATE itas_FlowCtrl SET ImageName='"+image_name+"'"
    _sql += " WHERE ZipFileName='"+zip_name+"'"
    sr.execute(_sql)
    sr.commit() 
    sr.close()
...
def FlowCtrl_Step_4(zip_name) :
    cn = pyodbc.connect(sys_args['flowConnectStr'],autocommit=True)
    sr = cn.cursor()
    _sql = "UPDATE itas_FlowCtrl SET Progress='4:移16bit存放',Status='1:處理中'"
    _sql += " WHERE ZipFileName='"+zip_name+"'"
    sr.execute(_sql)
    sr.commit() 
    sr.close()
...
def FlowCtrl_Step_4_Store(zip_name,file_store) :
    cn = pyodbc.connect(sys_args['flowConnectStr'],autocommit=True)
    sr = cn.cursor()
    _sql = "UPDATE itas_FlowCtrl SET FileStore_16='"+file_store+"'"
    _sql += " WHERE ZipFileName='"+zip_name+"'"
    sr.execute(_sql)
    sr.commit() 
    sr.close()
...
def FlowCtrl_Step_5(zip_name) :
    cn = pyodbc.connect(sys_args['flowConnectStr'],autocommit=True)
    sr = cn.cursor()
    _sql = "UPDATE itas_FlowCtrl SET Progress='5:加入16bitMosaicDataset',Status='1:處理中'"
    _sql += " WHERE ZipFileName='"+zip_name+"'"
    sr.execute(_sql)
    sr.commit() 
    sr.close()
...
def FlowCtrl_Step_5_MosaicDataset(zip_name,mosaic_dataset,image_name,raster_id,image_path) :
    cn = pyodbc.connect(sys_args['flowConnectStr'],autocommit=True)
    sr = cn.cursor()
    _sql = "UPDATE itas_FlowCtrl SET MosaicDataset_16='"+mosaic_dataset+"'"
    _sql += " WHERE ZipFileName='"+zip_name+"'"
    sr.execute(_sql)
    sr.commit() 
    sr.close()
...
def FlowCtrl_Step_6(zip_name) :
    cn = pyodbc.connect(sys_args['flowConnectStr'],autocommit=True)
    sr = cn.cursor()
    _sql = "UPDATE itas_FlowCtrl SET Progress='6:Pen-Sharpening',Status='1:處理中'"
    _sql += " WHERE ZipFileName='"+zip_name+"'"
    sr.execute(_sql)
    sr.commit() 
    sr.close()
...
def FlowCtrl_Step_7(zip_name) :
    cn = pyodbc.connect(sys_args['flowConnectStr'],autocommit=True)
    sr = cn.cursor()
    _sql = "UPDATE itas_FlowCtrl SET Progress='7:16bit轉8bit',Status='1:處理中'"
    _sql += " WHERE ZipFileName='"+zip_name+"'"
    sr.execute(_sql)
    sr.commit() 
    sr.close()
...
def FlowCtrl_Step_8(zip_name) :
    cn = pyodbc.connect(sys_args['flowConnectStr'],autocommit=True)
    sr = cn.cursor()
    _sql = "UPDATE itas_FlowCtrl SET Progress='8:移8bit存放',Status='1:處理中'"
    _sql += " WHERE ZipFileName='"+zip_name+"'"
    sr.execute(_sql)
    sr.commit() 
    sr.close()
...
def FlowCtrl_Step_8_Store(zip_name,file_store) :
    cn = pyodbc.connect(sys_args['flowConnectStr'],autocommit=True)
    sr = cn.cursor()
    _sql = "UPDATE itas_FlowCtrl SET FileStore_8='"+file_store+"'"
    _sql += " WHERE ZipFileName='"+zip_name+"'"
    sr.execute(_sql)
    sr.commit() 
    sr.close()
...
def FlowCtrl_Step_9(zip_name) :
    cn = pyodbc.connect(sys_args['flowConnectStr'],autocommit=True)
    sr = cn.cursor()
    _sql = "UPDATE itas_FlowCtrl SET Progress='9:加入8bitMosaicDataset',Status='1:處理中'"
    _sql += " WHERE ZipFileName='"+zip_name+"'"
    sr.execute(_sql)
    sr.commit() 
    sr.close()
...
def FlowCtrl_Step_9_MosaicDataset(zip_name,mosaic_dataset,image_name,raster_id,image_path) :
    cn = pyodbc.connect(sys_args['flowConnectStr'],autocommit=True)
    sr = cn.cursor()
    _sql = "UPDATE itas_FlowCtrl SET MosaicDataset_8='"+mosaic_dataset+"'"
    _sql += " WHERE ZipFileName='"+zip_name+"'"
    sr.execute(_sql)
    sr.commit() 
    sr.close()
...
def FlowCtrl_Step_99(zip_name, end_time, raster_type_id) :
    ...
    _sql = "UPDATE itas_FlowCtrl SET Progress='99:完成',Status='2:成功',EndTime='"+end_time+"',Priority=" + sys_args['defaultPriority']
    _sql += " WHERE ZipFileName='"+zip_name+"'"
    sr.execute(_sql)
    sr.commit() 
    sr.close()
...
def checkCreateFileStore(zip_name) :
    ...
    cn = pyodbc.connect(sys_args['flowConnectStr'],autocommit=True)
    sr = cn.cursor()
    _sql = "select T50YearMonth from itas_FlowCtrl"
    _sql += " where ZipFileName='" + zip_name + "'"
    sr.execute(_sql)
    rows = sr.fetchall()
    if (len(rows)>0) :
...
def main():
    ...
    #while True :
       ...
       try:
           ...
           #本查路徑中檔案，但此處用 zip_name 第一欄星種來載入參數
           if checkImageType(zip_name):
           ...
           # 自行 pansharpen 要此時 pansharpen 到 C: AutoPan(處理 BlackSky PHR 等非制式星種)
           ...
           if nowImage_args['bits'] == '16' :
               ...
               # 取 16bit NAS 路徑 MetaData MUL 及 Pan 檔名後用
               ...
               # 自行 pansharpen 用 raster dataset AutoPan 加入(處理 BlackSky PHR 等無此星種)
               if nowImage_args['self_pansharpen'] == 'Y' :
                   ...
                   # 非自行 pan 則為制式星種，用 xml_raster_type 
               ...
               move_zip_to_broken(sys_args['zips_path']+zip_file_name, zip_file_name, 'test')
...
def addRunTimes() :
    # 先鎖定再寫入，避免同時寫入
    lock_times = FileLock("itas_lock_times.txt.lock")
    try:
...
if haveFullRun() :
    ...
    if sys_args['is_MultiRun'] != 'Y' :         # 獨佔模式要用鎖定
        lock = FileLock("itas_high_ground.txt.lock")
        try:
