...
def checkImageType(image_root_path):
    ...
    if not bo : bo = checkPlanetScope( image_root_path )        # 找 PlanetScope
    return bo
...
def getMetaDataName( sour_imagePath ) :
    ...
    if nowImage_args['rasterType_name'] in ['BlackSky'] :
        ...
        for root, dirs, files in walk(sour_imagePath):
            for f in files:
                fullpath = join(root, f)
                if f.endswith('.tif') and not f.endswith('-pan.tif'):     # tif 檔且中間無 pan 字樣
                    nowImage_args['pathMUL'] = fullpath
                    bo = True
                    break
            if bo :
...
def checkCreateFileStore(zip_name) :
    # nowImage_args['fileStore_16'] 改依 sys_args['store_root_path']+年月+星種+16bit
    ...
    # 年月空白自動建立
    theDate = datetime.now()
    theDateS = theDate.strftime('%Y%m')
    # 由存放根路徑開始
    path = sys_args['store_root_path'] + '\\' + theDateS
    if not os.path.exists(path) :
        os.mkdir(path)
    # 再檢查星種自動建立
    path += '\\'+nowImage_args['rasterType_id']
    if not os.path.exists(path) :
        os.mkdir(path)
    # 檢查 16bit 自動建立
    if not os.path.exists(path+'\\16bit') :
        os.mkdir(path+'\\16bit')
    # nowImage_args['fileStore_16'] 此時放入
    nowImage_args['fileStore_16'] = path+'\\16bit'
    # 檢查 8bit 自動建立
    if not os.path.exists(path+'\\8bit') :
        os.mkdir(path+'\\8bit')
    # nowImage_args['fileStore_8'] 此時放入
    nowImage_args['fileStore_8'] = path+'\\8bit'
    ...
    while True :
       ...
       try:
           ...
           if nowImage_args['bits'] == '16' :
               ...
               # pan-sharpen 提前執行，以因應無 addraster autopansharpen 需自行 pansharpen 圖檔
