...
# 系統參數檔
syspara_path = sys_path + 'upd_sysparam.csv'
sys_args = {
    ...
    'rasterType_filt': 'WorldView-3'                    # 指定重新轉入 MD 的星種，無則是全部
}
...
def fprint(w_str) :
    ...
    with open(sys_path+'upd_log.txt', 'a',encoding='UTF-8') as f:
...
def load_sys_param():
    ...
    # 解譯到 sys_args
    sys_args['flowConnectStr']  = lines[0][0]
    sys_args['config_path']     = lines[0][1]
    sys_args['rasterType_filt'] = lines[0][2]
...
def FlowCtrl_Step_9_MosaicDataset(zip_name,mosaic_dataset,image_name,raster_id,image_path) :
    ...
    for row in rows:
        ...
        if nowImage_args['rasterType_name'] in ['Other'] :
            row.setValue("img_id",              image_MetaData['img_id']     )
            row.setValue("acq_time",            image_MetaData['acq_time']   )
            row.setValue("sun_elev",            image_MetaData['sun_elev']   )
            row.setValue("sun_azu",             image_MetaData['sun_azu']    )
            row.setValue("cloud_rate",          image_MetaData['cloud_rate'] )
            row.setValue("band",                image_MetaData['band']      )
            row.setValue("cen_x",               image_MetaData['cen_x']      )
            row.setValue("cen_y",               image_MetaData['cen_y']      )
            row.setValue("ul_x",                image_MetaData['ul_x']       )
            ...
            row.setValue("lr_x",                image_MetaData['lr_x']       )
            row.setValue("lr_y",                image_MetaData['lr_y']       )
            row.setValue("sat_type",            image_MetaData['sat_type']   )
            row.setValue("catalog_id",          image_MetaData['catalog_id'] )
            row.setValue("receive_time",        image_MetaData['receive_time']   )
