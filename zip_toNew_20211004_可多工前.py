    ...
    for f in arr:
        ...
        for f_sub in arr_sub:
           ...
           if not os.path.isdir( dir_name ):
               continue
           # 檢查 history，不存在即可壓縮
           if dir_name not in histlines:
               return f+','+f_sub                                       # 將年月路徑及壓縮路徑名回傳
...
def CompressDir(dir_name,dir_year_month) :
    ...
    zf = zipfile.ZipFile((sys_path+'{}.zip').format(dir_name), 'w', zipfile.ZIP_DEFLATED)
    ...
    shutil.move( sys_path+dir_name+'.zip', move_path+dir_name+'.zip' )
...
# 僅可一份檢查
lock = FileLock("zip_ground.txt.lock")
try:
