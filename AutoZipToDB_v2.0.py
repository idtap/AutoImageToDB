...
# 定義流程控制 connect string( MS-Access .mdb)
...
def getNextZip() :
    ...
    for zip_name in arr_zip:
        ...
        # 無此 zip 則 LOG 新增一筆
        if (len(rows)<=0) :
            _sql = "INSERT INTO FlowCtrl VALUES('"+zip_name+"','',9999,'0:待辦',0,'','','','',16)"
            sr.execute(_sql)
            if (sr.rowcount <= 0) :
...
def FlowCtrl_Step_1(zip_name, start_time) :
    ...
    cn = pyodbc.connect(flowConnectStr)
    sr = cn.cursor()
    _sql = "UPDATE FlowCtrl SET Progress='1:獲優先開始轉',Status=2,StartTime='"+start_time+"'"
    _sql += " WHERE ZipFileName='"+zip_name+"'"
    sr.execute(_sql)
    sr.commit() 
    sr.close()
...
def FlowCtrl_Step_2(zip_name) :
    cn = pyodbc.connect(flowConnectStr)
    sr = cn.cursor()
    _sql = "UPDATE FlowCtrl SET Progress='2:星種判讀',Status=1"
    _sql += " WHERE ZipFileName='"+zip_name+"'"
    sr.execute(_sql)
    sr.commit() 
    sr.close()
    ...
    return True
# 紀錄星種欄
def FlowCtrl_Step_2_RasterType(zip_name, raster_type) :
    cn = pyodbc.connect(flowConnectStr)
    sr = cn.cursor()
    _sql = "UPDATE FlowCtrl SET RasterType='"+raster_type+"'"
    _sql += " WHERE ZipFileName='"+zip_name+"'"
    sr.execute(_sql)
    sr.commit() 
    sr.close()
...
def FlowCtrl_Step_3(zip_name) :
    cn = pyodbc.connect(flowConnectStr)
    sr = cn.cursor()
    _sql = "UPDATE FlowCtrl SET Progress='3:圖名檢查',Status=1"
    _sql += " WHERE ZipFileName='"+zip_name+"'"
    sr.execute(_sql)
    sr.commit() 
    sr.close()
...
def FlowCtrl_Step_4(zip_name) :
    cn = pyodbc.connect(flowConnectStr)
    sr = cn.cursor()
    _sql = "UPDATE FlowCtrl SET Progress='4:移16bit存放',Status=1"
    _sql += " WHERE ZipFileName='"+zip_name+"'"
    sr.execute(_sql)
    sr.commit() 
    sr.close()
...
def FlowCtrl_Step_5(zip_name) :
    cn = pyodbc.connect(flowConnectStr)
    sr = cn.cursor()
    _sql = "UPDATE FlowCtrl SET Progress='5:加入16bitMosaicDataset',Status=1"
    _sql += " WHERE ZipFileName='"+zip_name+"'"
    sr.execute(_sql)
    sr.commit() 
    sr.close()
...
def FlowCtrl_Step_6(zip_name) :
    cn = pyodbc.connect(flowConnectStr)
    sr = cn.cursor()
    _sql = "UPDATE FlowCtrl SET Progress='6:Pen-Sharpening',Status=1"
    _sql += " WHERE ZipFileName='"+zip_name+"'"
    sr.execute(_sql)
    sr.commit() 
    sr.close()
...
def FlowCtrl_Step_7(zip_name) :
    cn = pyodbc.connect(flowConnectStr)
    sr = cn.cursor()
    _sql = "UPDATE FlowCtrl SET Progress='7:16bit轉8bit',Status=1"
    _sql += " WHERE ZipFileName='"+zip_name+"'"
    sr.execute(_sql)
    sr.commit() 
    sr.close()
...
def FlowCtrl_Step_8(zip_name) :
    cn = pyodbc.connect(flowConnectStr)
    sr = cn.cursor()
    _sql = "UPDATE FlowCtrl SET Progress='8:加入8bitMosaicDataset',Status=1"
    _sql += " WHERE ZipFileName='"+zip_name+"'"
    sr.execute(_sql)
    sr.commit() 
    sr.close()
...
# Step 9 完成
def FlowCtrl_Step_9(zip_name, end_time) :
    cn = pyodbc.connect(flowConnectStr)
    sr = cn.cursor()
    _sql = "UPDATE FlowCtrl SET Progress='99:完成',Status=2,EndTime='"+end_time+"'"
    _sql += " WHERE ZipFileName='"+zip_name+"'"
    sr.execute(_sql)
    sr.commit() 
    sr.close()
    ...
    while 1 :
       ...
       try:
           ...
           if checkImageType(tempzip_dir):
              ...
              FlowCtrl_Step_2_RasterType(zip_name, nowImage_args['rasterType_name'])
              FlowCtrl_StatusMsg(zip_name, '2','')
           else:
              fprint('此 zip 圖檔無法判讀星種\n')
              FlowCtrl_StatusMsg(zip_name, '3','不明星種')
              continue
           ...
           if not getMetaDataPath( tempzip_dir ) :
               ...
               FlowCtrl_StatusMsg(zip_name, '3', '找不到圖名')
               continue
           ...
           # 完成
           FlowCtrl_Step_9(zip_name,theTime_e.strftime('%Y/%m/%d %H:%M:%S'))

       except Exception as e:
           fprint('\n發生錯誤:')
           fprint(e)
           FlowCtrl_StatusMsg(zip_name, '3', '錯誤:'+e)
