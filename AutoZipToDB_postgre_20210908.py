...
def parserWorldViewXML( xml_file_name ) :
    ...
    image_MetaData['acq_time'] = getXmlTagValue(root,'./IMD/MAP_PROJECTED_PRODUCT/EARLIESTACQTIME')
    ...
    image_MetaData['gen_time'] = getXmlTagValue(root,'./IMD/GENERATIONTIME')
...
def parserMetaData() :  
    ...
    # parser 有 imd xml 兩種檔案方式(有 .xml 時優先)
    imd_name = nowImage_args['pathMUL']
    xml_name = os.path.splitext(imd_name)[0] + ".xml"

    # 兩檔案都不存在時，回錯誤
    if not os.path.exists(imd_name) and not os.path.exists(xml_name) :
        return False

    # 依各星種 parser
                                          # World-View 目前都相同
    if nowImage_args['rasterType_name'] in ['WorldView-1','WorldView-2','WorldView-3','WorldView-4'] :
        # xml 優先
...
def move_zip_to_stub(sour_zip, zip_file_name) :
    ...
    fprint( '轉完，將 zip 檔移動到:'+move_path+'/'+zip_file_name+'\n' )
